"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.SeatSchema = exports.Seat = void 0;
const mongoose_1 = require("@nestjs/mongoose");
const mongoose_2 = require("mongoose");
const seat_enum_1 = require("../types/seat.enum");
let Seat = class Seat {
    seatNumber;
    lab;
    status;
};
exports.Seat = Seat;
__decorate([
    (0, mongoose_1.Prop)({ required: true }),
    __metadata("design:type", String)
], Seat.prototype, "seatNumber", void 0);
__decorate([
    (0, mongoose_1.Prop)({ type: mongoose_2.Types.ObjectId, ref: 'Lab', required: true }),
    __metadata("design:type", mongoose_2.Types.ObjectId)
], Seat.prototype, "lab", void 0);
__decorate([
    (0, mongoose_1.Prop)({ type: String, enum: seat_enum_1.SeatStatus, default: seat_enum_1.SeatStatus.AVAILABLE }),
    __metadata("design:type", String)
], Seat.prototype, "status", void 0);
exports.Seat = Seat = __decorate([
    (0, mongoose_1.Schema)()
], Seat);
exports.SeatSchema = mongoose_1.SchemaFactory.createForClass(Seat);
//# sourceMappingURL=seat.schema.js.map